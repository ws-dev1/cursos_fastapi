...
class UserUseCases:
    ...
    def user_login(self, user: User, expires_in: int = 30):
        user_on_db = self.get_user(user.username)
        if user_on_db is None:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='username or password not already')

        if not crypt_context.verify(user.password, user_on_db.password):
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='username or password not already')

        expires_at = datetime.utcnow() + timedelta(30)
    ...
    def verify_token(self, token: str):
        ...
        user_on_db = self.get_user(username=data['sub'])
